...
\1 Given that $\tan{2x}=a_{0}+a_{1}x+a_{2}x^2+...$, by using the fact that $\sin{2x}=\cos{2x}\cdot \tan{2x}$, obtain the expansion of $\tan{2x}$ in ascending powers of $x$, up to and including the term in $x^3$.\hfill[4] %Question 4

\1 It is given that $\textrm{f}(x)=\dfrac{a-b}{(1-ax)(1-bx)}$, where $a,b>0$. %Question 5
